...
async function waitForBuild(apiKey, username, domain, buildUuid, onLog) {
  for (let i = 0; i < 30; i++) {
    ...
    const builds = await api(apiKey, username, domain, `/api/hosting/v1/accounts/${username}/websites/${domain}/nodejs/builds?per_page=5`);
    ...
    if (state === "failed") {
      const logs = await api(apiKey, username, domain, `/api/hosting/v1/accounts/${username}/websites/${domain}/nodejs/builds/${buildUuid}/logs`).catch(() => null);
...
async function deployToKelvinoz(options = {}) {
  ...
  const domain = options.domain || process.env.HOSTINGER_DOMAIN || "kelvinoz.com";
  ...
  const build = await api(apiKey, username, domain, `/api/hosting/v1/accounts/${username}/websites/${domain}/nodejs/builds`, {
    method: "POST",
    body: JSON.stringify({
      node_version: 18,
      app_type: "express",
  ...
  await api(apiKey, username, domain, `/api/hosting/v1/accounts/${username}/websites/${domain}/cache/clear`, { method: "DELETE" }).catch(() => {});
  await api(apiKey, username, domain, `/api/hosting/v1/accounts/${username}/websites/${domain}/nodejs/server/restart`, {
    method: "POST",
    body: JSON.stringify({}),
  }).catch(() => {});

  return { ok: true, url: `https://${domain}`, buildId: build.uuid };
}

module.exports = { deployToKelvinoz };
